# ------------------------------------------------------------------------
# X-Plane import/output utility classes
# Based on XPlane2Blender plugin v3.10 by Jonathan Harris
#
...
class PanelRegionHandler:
    ...
    def New(self, panelimage):
        if self.obj:
            ...
            mesh = bpy.data.meshes.new(name=PanelRegionHandler.NAME)
            # self.obj=Mesh.New(PanelRegionHandler.NAME)
            # self.obj=bpy.data.meshes.new(name=PanelRegionHandler.NAME)
            # self.obj=Object.New('Mesh', PanelRegionHandler.NAME)
            self.obj = bpy.data.objects.new(name=PanelRegionHandler.NAME, object_data=mesh)
            # self.obj.link(mesh)
        ...
        #     mesh.face_maps.extend([[v,v+1,v+2]])
    ...
    def addRegion(self, xoff, yoff, width, height):
        ...
        for img in Image.get():
            ...
            if img.size == [width,
                            height] and img.source == Image.Sources.GENERATED and img.filename == name and not self.isRegion(
                    img):
                break
        else:
...
def getManipulators():
    ...
    """
    manipulators = {}
    cursors = ['four_arrows', 'hand', 'button', 'rotate_small', 'rotate_small_left', 'rotate_small_right',
               'rotate_medium', 'rotate_medium_left', 'rotate_medium_right', 'rotate_large', 'rotate_large_left',
               'rotate_large_right', 'up_down', 'down', 'up', 'left_right', 'right', 'left', 'arrow']

    manipulators['ATTR_manip_none'] = {'00@NULL': 0}
    manipulators['ATTR_manip_drag_xy'] = {'00@cursor': '', '01@dx': 0.0, '02@dy': 0.0, '03@v1min': 0.0, '04@v1max': 0.0,
                                          '05@v2min': 0.0, '06@v2max': 0.0, '07@dref1': '', '08@dref2': '',
                                          '09@tooltip': ''}
    manipulators['ATTR_manip_drag_axis'] = {'00@cursor': '', '01@dx': 0.0, '02@dy': 0.0, '03@dz': 0.0, '04@v1': 0.0,
                                            '05@v2': 0.0, '06@dataref': '', '07@tooltip': ''}
    manipulators['ATTR_manip_command'] = {'00@cursor': '', '02@command': '', '03@tooltip': ''}
    manipulators['ATTR_manip_command_axis'] = {'00@cursor': '', '01@dx': 0.0, '02@dy': 0.0, '03@dz': 0.0,
                                               '04@pos-command': '', '05@neg-command': '', '06@tooltip': ''}
    manipulators['ATTR_manip_noop'] = {'00@NULL': 0}
    manipulators['ATTR_manip_push'] = {'00@cursor': '', '01@v-down': 0.0, '02@v-up': 0.0, '03@dataref': '',
                                       '04@tooltip': ''}
    manipulators['ATTR_manip_radio'] = {'00@cursor': '', '01@v-down': 0.0, '02@dataref': '', '03@tooltip': ''}
    manipulators['ATTR_manip_toggle'] = {'00@cursor': '', '01@v-on': 0.0, '02@v-off': 0.0, '03@dataref': '',
                                         '04@tooltip': ''}
    manipulators['ATTR_manip_delta'] = {'00@cursor': '', '01@v-down': 0.0, '02@v-hold': 0.0, '03@v-min': 0.0,
                                        '04@v-max': 0.0, '05@dataref': '', '06@tooltip': ''}
    manipulators['ATTR_manip_wrap'] = {'00@cursor': '', '01@v-down': 0.0, '02@v-hold': 0.0, '03@v-min': 0.0,
                                       '04@v-max': 0.0, '05@dataref': '', '06@tooltip': ''}

    return (manipulators, cursors)

class CurrentRotate:
    def __init__(self, vector, dataRef):
        self.vector = vector
